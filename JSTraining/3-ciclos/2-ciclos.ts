...
// 2.  vamos a crear un objeto con los items de la conversion que nos piden convertir.
const convertidor: { [key: string]: string } = {
  A: "4",
  E: "3",
  I: "1",
};
...
for (let index = 0; index < frase.length; index++) {
  ...
  const conversion = convertidor[letra as keyof typeof convertidor];

  if (conversion !== undefined) {
    nuevoString += conversion;
  } else {
    nuevoString += letra;
  }
}
...

{
  ...
  const frase: string =
    "este es un tutorial de como utilizar ciclos y condicionales para crear unas logicas bien elaboradas.";
